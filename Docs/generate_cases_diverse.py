...
스펙:
  ...
  - 적재 부피 fill: 90%는 >50%, 10%만 ≤50% (너무 비지 않게). ※ 코일/파이프만은 태생적으로 낮음 → 그게 10%.
...
저장: CargoLayoutFile JSON + 메타(composition/cogMode/fill/cog/totalMass/cargoCount/maxHeight).
...
def main():
    ...
    for i in range(1, N_CASES + 1):
        ...
        print(f"case{i:03d}: {data['composition']:9s} {data['cogMode']:10s} "
              f"fill={data['fill'] * 100:4.1f}% mass={data['totalMass']:.2f}kg n={data['cargoCount']:2d} "
              f"CoG=({data['cog']['x']:+.3f},{data['cog']['y']:.3f},{data['cog']['z']:+.3f}) maxH={data['maxHeight'] * 100:.0f}cm")

    print(f"\n→ {OUT_DIR} (미리보기 {N_CASES}개)")
    print(f"fill>50% 비율: {hi_fill}/{N_CASES}  (목표 ≈90%)")
