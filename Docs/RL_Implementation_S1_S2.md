...
> ⚠️ **격자/관측 수치 스테일 (2026-07-08)**: 본문의 "현재 확정값 `1cm/1281셀`·`관측 1299`"는 **과거**입니다. 현재는 **2cm/341셀·관측 359**(고정 케이스 350). **현재 진실은 `STATUS.md` 상단 표 참조.** (S1/S2 규칙·보상 로직 설명은 유효.)
...
> ⚠️ **이 섹션의 값 일부는 이후 갱신됨(이 문서는 S1/S2 구현 당시 기록).** 현재 확정값은 항상 **`RL_Applied_Spec.md`(source of truth)** 기준: 적재함 길이 62→**61cm**, 격자 4cm/96셀→**1cm/1281셀**, 화물 종류 16→풀 **12종**, 관측 114→**1299**. 아래 표는 당시 스냅샷으로 남겨둔다.
