...
## 2026-07-07 — 다양 배치 데이터셋 500개 재생성 (예측기용)

- **아카이브**: 기존 `Cases/` 500개 → `Cases_shelf2000_archive_20260705/risk500_archived_20260707/` (손실 방지 이동).
- **새 생성기 `Docs/generate_cases_diverse.py`** (기존 `generate_risk_cases.py` 보존): 조합 다양 + CoG 다양 + **하드룰 준수(현실적, 의도적 위반 없음: ≤27cm·≤7kg·겹침X·받쳐짐·파이프눕힘·긴파이프만 뒤오버행)**. `pick_light`로 가벼운 화물 우선(무게예산 절약→아이템 다수).
- **결과 500개**: 조합 mixed198/box_only97/box_pipe92/box_light63/pipe27/coil23. CoG모드 low_center172/high81/ecc_L91/ecc_R82/front_back74. fill 평균28%(최대55). 화물수 평균23. mass≤7 전부. CoG x±0.07·y0.02~0.18·z±0.25.
- ⚠️ **fill>50%는 13개뿐** — 랜덤 비겹침 배치의 부피천장 ~48%(틈). 사용자와 합의: 예측기엔 부피%보다 **다양성**이 핵심이라 수용(시각적 꽉참은 개수/높이로 달성). 단 coil/pipe_only 45개는 <5개=희소(비어보임) — 필요시 비율 조정.
- **JSON 메타**(composition/cogMode/fill/cog{x,y,z}/totalMass/cargoCount/maxHeight) 포함 → 나중 시계열 CSV 컬럼용 그대로.
- **다음**: 500개 배치 주행(batch, runAllCases) → results/시계열 → 예측기. (+ 시계열 CSV에 배치특징 컬럼 추가 = DataLogger.BeginRun.)
