...
/// <summary>
/// PlacementAgent 의 현재 배치(placed)를 실제 화물 모양으로 그려주는 "표시 전용" 시각화.
/// 학습/보상/관측엔 전혀 영향 없음 — 눈으로 보라고 메시만 띄운다.
///
/// - 화물 = 정적/동적 씬과 동일한 <see cref="CargoFactory"/> 로 생성 (파이프=원통, 코일=도넛,
///   포대=빵빵한 자루형, 드럼=원통, 박스=큐브) + 실물 재질까지 그대로.
/// - 적재함(21×27×62cm) = 바닥판 + 와이어 박스 외곽 + 6×16 그리드 (RL 격자와 동일).
/// - 무게중심(CoG) = 빨간 구 + 바닥까지 수직선 (실시간).
/// - 회전(rot90)은 placed 의 halfSize 로 역산해 반영.
/// - 작아서 displayScale 로 키워 표시 (기본 10배).
///
...
public class PlacementVisualizer : MonoBehaviour
{
    ...
    void Start()
    {
        var go = new GameObject("PlacementViz_Root");
        root = go.transform;
    ...
    void BuildCornerLabels()
    {
        ...
        float lx = cfg.trayLateralM, lz = cfg.trayLengthM, y = cfg.floorTopY + 0.008f;
    ...
    void BuildTray()
    {
        ...
        // 외곽 와이어 박스 (적재 볼륨 12모서리) — x∈[0,lx], z∈[0,lz]
        ...
        // 6×16 그리드 (RL 격자 = 에이전트가 고르는 96셀) — 코너 원점 [0,lx]·[0,lz]
        if (showGrid)
        {
    ...
    void BuildHeightLimit()
    {
        ...
        float cx = lx * 0.5f, cz = lz * 0.5f;   // 중심 (코너 원점 기준)
        float topY = cfg.floorTopY + cfg.heightLimitM;   // 한도 평면 y (=0.28)

        // (B) 반투명 빨간 천장면 (화물이 뚫으면 바로 보임)
        ...
        // (A) 상단 빨간 굵은 테두리 (4모서리 바) — x∈[0,lx], z∈[0,lz]
    ...
    static Material MakeTransparent(Color c)
    {
        ...
        s.SetFloat("_Mode", 3f);                   // Transparent
    ...
    void SetupCamera()
    {
        var camGo = new GameObject("PlacementViz_Camera");
        ...
        Vector3 look = new Vector3(agent.ruleConfig.trayLateralM * 0.5f, agent.ruleConfig.floorTopY + 0.05f, lenZ * 0.5f); // 트레이 중심
    ...
    void OnDrawGizmos()
    {
        ...
        Gizmos.DrawWireCube(new Vector3(cfg.trayLateralM * 0.5f, cfg.floorTopY + cfg.heightLimitM * 0.5f, cfg.trayLengthM * 0.5f),
