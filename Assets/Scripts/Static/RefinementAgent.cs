...
/// - 행동: (아이템 index · 목표 셀 · 회전2 · 모드[이동/스왑] · 스왑상대) — item 하나를 옮기거나 두 item을 맞바꿈
...
public class RefinementAgent : Agent
{
    ...
    [Tooltip("파일명 접두사 's0_'·'s1_'·'s2_'·'s3_'로 소속 단계를 구분한다. " +
             ...
             "실제 이번 런이 몇 단계를 학습하는지는 아래 currentStage로 따로 지정한다. 비우면 startManifest 1개로 폴백.")]
    ...
    [Tooltip("커리큘럼 전이(--initialize-from)용: 행동공간(아이템 수)을 이 값으로 고정. 0=자동(케이스 최대·4단계 최대치). " +
             "커리큘럼이면 단계마다 케이스가 달라도 action space가 같아야 전이가 되므로, " +
             "전체 단계에서 쓸 케이스들의 최대 아이템 수(예: 20)로 모든 단계에 동일하게 설정.")]
    ...
    [Range(0f, 1f)] public float stage4ReplayRatio = 0.3f;   // 0.15→0.3: s4 ValidMoveRate 정체(0.3) 구제 — s0~3 앵커링↑
    ...
    private int curCaseIdx;                                                 // 이번 에피소드 케이스 index (4단계 절차생성이면 -1)
    ...
    private void Setup()
    {
        ...
        // 항상 감안한다 — currentStage가 몇이든 무관하게 always 최대로 잡아야, 앞으로 이 컴포넌트로 어떤 단계를
        ...
        autoMax = Mathf.Max(autoMax, stage4TotalItemsRange.y);
        if (maxItemsOverride > 0)
        {
            numItems = maxItemsOverride;
            if (maxItemsOverride < autoMax)
                Debug.LogWarning($"[Refine] maxItemsOverride({maxItemsOverride}) < 케이스·4단계 최대 아이템({autoMax}) → 초과 아이템 선택 불가. override를 {autoMax} 이상으로 설정하세요.");
        }
        ...
        Debug.Log($"[RefinementAgent] obs={ObsSize}, action=({numItems},{NumCells},2,2,{numItems})[이동+스왑], 케이스={caseSet.Count}개" +
